...
public class WindowJumpJourney : Journey
{
    ...
    public WindowJumpJourney(Person person, Floor floor, float walkSpeed) :
    ...
    protected override void Travel(bool finishMovement, bool finishRotation) {
        if (!thrown) {
            //start running
            if (!passengerAnimator.IsAtState(PersonAnimator.RUN)) {
                passengerAnimator.Activate(PersonAnimator.RUN);
                Speed *= person.RunSpeedMultiplier;
            }

            //jump at window
            if (finishMovement) {
                passengerAnimator.Activate(PersonAnimator.JUMP, true);
                passengerAnimator.Activate(PersonAnimator.RUN, false);
                ContinuePath();
                thrown = true;
            ...
            //fall
            if (!fall) {
                passengerAnimator.Activate(PersonAnimator.FALL, true);
                passengerAnimator.Activate(PersonAnimator.JUMP, false);
                Speed = FALL_SPEED;
                ContinuePath();
                fall = true;
            }
            //crash
                ...
                passengerAnimator.Activate(PersonAnimator.CRASH, true);
                passengerAnimator.Activate(PersonAnimator.FALL, false);
                ContinuePath();
            }
        }
    }
    ...
    protected override Queue<Vector3> GeneratePath() {
        ...
        Vector3 passengerPos = person.transform.position;
