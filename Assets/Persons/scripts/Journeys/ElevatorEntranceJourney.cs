...
public class ElevatorEntranceJourney : Journey
{
    ...
    public ElevatorEntranceJourney(Person person, Floor floor, float walkSpeed) :
    ...
    public override bool Activate() {
        ...
        if (isActivated) floorplan.Free(person.LeasedPath);
        return isActivated;
    }

    protected override Queue<Vector3> GeneratePath() {
        Queue<Vector3> path = new Queue<Vector3>();
        selectedElevator = ((Passenger) person).TargetElevatorBuffer;
    ...
    public override void ContinuePath() {
        ...
        if (path.Count == 0 && !lookAtElevator) {
            ...
            targetPos.y = person.transform.position.y;
